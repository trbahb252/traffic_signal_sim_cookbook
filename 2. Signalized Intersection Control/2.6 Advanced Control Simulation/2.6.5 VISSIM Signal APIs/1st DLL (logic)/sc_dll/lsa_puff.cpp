...
//  plp = new T_LSA_Puffer ();
  ...
  /* Speicherplatz fuer Signalgruppen belegen: */
    ...
    plp->SG_Name_to_Index = new NameToIndexMap ();
...
int sg_name_to_index (const std::string & vissim_name, char* function_name)
{
  /* Gibt den internen Signalgruppenindex der Signalgruppe mit dem gegebenen */
  /* Namen zurück falls er existiert, sonst -1.                              */
  int   result;
  NameToIndexMap::iterator iter = LSA_Puffer->SG_Name_to_Index->find(vissim_name);

  if (iter == LSA_Puffer->SG_Name_to_Index->end()) {
    if (function_name != NULL) {
      std::stringstream reason;
      reason << "No signal group with name \"" << vissim_name << "\" found.";
      InternerFehler (function_name, reason.str().c_str());
    }
...
int StringToLSAPuffer (long typ, long nummer, std::string& wert)
{
  ...
  switch (typ) {
    ...
    case SC_DLL_DATA_SG_NAME:
      ...
      if (index >= 0) {
        LSA_Puffer->SigGr[index]->Name = wert;

        if (!wert.empty() && 
          LSA_Puffer->SG_Name_to_Index->find(wert) == LSA_Puffer->SG_Name_to_Index->end()) {
            LSA_Puffer->SG_Name_to_Index->insert(std::make_pair(wert, index));
        }
      }
      break;
    default:
...
void  VonLSA (int kommando, int nr, BOOLEAN uebergang)
{
  ...
  if (kommando == 0) {
    LSA_Puffer->SigGr[index]->Soll = undefiniert;
    return;
  }
...
std::string sg_name(int nr)
{
  char  prozedurname[] = "LSA_PUFF.sg_name";
  int   index = sg_nr_to_index (nr, prozedurname, false);
